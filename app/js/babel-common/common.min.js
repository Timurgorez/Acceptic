...
window.onload = function () {
	...
	document.forms.reg.onsubmit = function () {
		...
		if (/^[a-zA-Z0-9а-яА-ЯёЁ']{2,15}?$/.test(name)) {
			...
			alert("Вы ввели не правильно имя");return false;
		}
		if (/^[A-z0-9._-]+@[A-z0-9.-]+\.[A-z]{2,4}$/.test(email)) {
			//alert("Mail OK");
		} else {
			alert("Вы ввели не правильно почту");return false;
		}
		if (/^[A-z0-9._-]{6,10}?$/.test(password)) {
			//alert("Password OK");
		} else {
			alert("Вы ввели не правильно пароль");return false;
		}
	...
	document.forms.change.onsubmit = function () {
		...
		var checked = document.getElementById('checkMail');
		var checkMail = checked.innerHTML;
